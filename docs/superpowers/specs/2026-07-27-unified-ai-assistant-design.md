...
Три прохода по уже загруженному в процессе GLiNER2
(`fact_relations.extractor.get_model()`, `fastino/gliner2-multi-v1`,
многоязычная): две классификации с разными формулировками меток, чьи скоры
ансамблируются, плюс один проход за сущностями и структурой. ~0.5 с CPU и ноль
LLM-токенов вместо целого вызова `CLASSIFICATION_SYSTEM_PROMPT`.
...
**Три решения ниже — замеренные, а не угаданные** (прод, 27.07.2026, 27 фраз
ru/en; фикстура — `tests/docker/test_assistant_routing.py`):

1. **Классификация живёт в отдельной схеме.** Добавление секций entities и
   structure в ту же схему роняет argmax-точность с 81% до 70% и разрушает
   разделимость скоров, на которой держится порог. Второй проход стоит ~200 мс
   и возвращает и то, и другое.
2. **Две формулировки меток, ансамбль.** Терсный набор и набор «the user is
   trying to…» ошибаются на разных фразах; нормированные и сложенные они дают
   89% argmax против 81% у одного набора.
3. **Порог — по ДОЛЕ, а не по абсолютной уверенности.** При `multi_label=True`
   скоры это независимые сигмоиды и гуляют в 0.02–0.96 в зависимости от
   формулировки, так что абсолютного порога, разделяющего классы, не
   существует. Доля победителя от суммы — существует: при `MIN_SHARE = 0.45`
   фикстура даёт 22/27 верных маршрутов при **нуле** неверных, остальные 5
   переспрашивают.
...
| Условие | Действие |
|---|---|
| явный `intent` в теле (ответ на `clarify`) | используется как есть, GLiNER не вызывается |
| распознано число ≥ 2 | жёсткий оверрайд → `playlist` (сигнал не языковой) |
| ≤ 3 слов и **не названо ничего** (ни артиста, ни песни, ни числа), есть `last_intent` | липкость: тот же интент |
| доля победителя ≥ `MIN_SHARE` | маршрутизируем |
| иначе | фрейм `clarify` с тремя кнопками |

### Липкость follow-up: почему правило в коде

«а побыстрее?» после плейлиста классификатор читает как **уверенный** поиск
(доля 0.51–0.63) и уходит искать в текстах слово «быстро». Липкость как
фоллбэк под порогом при этом не срабатывает никогда — отсюда правило стоит
до порога.

Обе модельные альтернативы замерены и отвергнуты:

- **4-я метка «уточнение предыдущего запроса»** размывает трёхсторонний
  ансамбль: «саундтрек к Интерстеллару» и «песни из GTA Vice City» начинают
  уверенно уезжать в search (0.574), чего без неё не было;
- **отдельный бинарный проход** «самостоятельный запрос против модификатора»
  не даёт разделения ни на одном пороге: минимум по follow-up 0.68 против
  максимума по обычным запросам 0.86. У GLiNER нет понятия диалогической
  отсылки.

Что модель **видит** — что сообщение не называет ничего. Искать в нём нечего,
значит это может быть только «то же самое, но иначе». Все короткие настоящие
запросы в фикстуре кого-то называют («собери хиты Канье», «лучшее у Queen»,
«биография Radiohead») — проверка сущностей их и отсекает.

**Известное ограничение, принято сознательно:** «а побыстрее?» как *первое*
сообщение уходит в search и отдаёт нерелевантное. Отправлять такие фразы в
`clarify` означало бы делать это для любого короткого запроса без сущностей,
включая законные «что-нибудь бодрое».
...
Правило слияния одно: **слоты всегда переносятся, новые сущности
перезаписывают**. Решать «это follow-up или нет» не нужно — если в «ещё у этого
артиста» GLiNER не нашёл артиста, берётся `last_artist`.

Единственное исключение — `last_song` пишется только на ходах `search`/`facts`:
на playlist-ходе GLiNER размечает сам текст желания как название песни
(«спокойного джаза на вечер»), и позднее «расскажи про эту песню» разрешилось
бы в бессмыслицу. Артист пишется всегда — там он настоящий.
