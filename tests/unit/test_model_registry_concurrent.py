...
def test_concurrent_loads_share_one_instance(monkeypatch):
    _reset_registry()
...
def test_get_text_model_returns_cached_triple_without_reloading(monkeypatch):
    _reset_registry()
    _FakeSentenceTransformer.instance_count = 0
    monkeypatch.setattr(
        "app.resources.model_registry.SentenceTransformer",
...
def test_get_text_model_two_distinct_models_two_instances(monkeypatch):
    _reset_registry()
    _FakeSentenceTransformer.instance_count = 0
    monkeypatch.setattr(
        "app.resources.model_registry.SentenceTransformer",
