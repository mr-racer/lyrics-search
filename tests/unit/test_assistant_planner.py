...
from app.services.assistant.contracts import Abbreviation
from app.services.assistant.planner import (Planner, parse_abbreviation, parse_era,
...
class TestValidate:
    ...
    def test_the_intent_is_what_drives_the_branch(self):
        """``allowed_tools`` did not travel to production: nothing ever read it,
        and the branch is chosen from the intent alone."""
        plan = _planner().validate({"intent": "general"}, "почему Eminem так зовут")
        assert plan.intent == "general"
        assert not hasattr(plan, "allowed_tools")
