"""Tests for audiodb_service: HTTP fetch, per-artist enrichment, and slug helpers."""
import asyncio
import pytest
import requests
...
from app.services.audiodb_service import (
    _audiodb_slug,
    _canonical_artist_name,
    _download_image,
    _http_get_json,
    fetch_audiodb_for_artist,
...
class TestAudiodbService:
    def test_http_get_json_happy_path(self):
    ...
    def test_http_get_json_retries_on_connection_error(self):
    ...
    def test_http_get_json_returns_none_after_two_failures(self):
    ...
    def test_http_get_json_returns_none_on_http_404_no_retry(self):
    ...
    def test_download_image_writes_content_addressed_file(self, tmp_path, monkeypatch):
    ...
    def test_download_image_returns_none_on_empty_url(self):
        result = _run(_download_image(None))
        assert result is None
        result = _run(_download_image(""))
        assert result is None

    def test_download_image_returns_none_on_http_failure(self, tmp_path, monkeypatch):
        monkeypatch.setattr("app.services.audiodb_service.ARTIST_COVERS_DIR", tmp_path)
        with patch(
            "app.services.audiodb_service.requests.get",
    ...
    def test_download_image_dedups_by_content(self, tmp_path, monkeypatch):
    ...
    def test_fetch_audiodb_for_artist_happy_path(self, isolated_db, tmp_path, monkeypatch):
        monkeypatch.setattr("app.services.audiodb_service.ARTIST_COVERS_DIR", tmp_path)
    ...
    def test_fetch_audiodb_for_artist_skips_if_already_fetched(self, isolated_db, tmp_path, monkeypatch):
        monkeypatch.setattr("app.services.audiodb_service.ARTIST_COVERS_DIR", tmp_path)
        # Pre-populate
    ...
    def test_deezer_fallback_when_audiodb_misses(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_deezer_fallback_when_audiodb_image_downloads_fail(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_deezer_skipped_when_audiodb_image_downloaded(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_deezer_skipped_when_only_cutout_downloaded(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_deezer_fallback_rejects_name_mismatch(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_deezer_fallback_name_match_is_case_insensitive(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_deezer_fallback_survives_deezer_failure(self, isolated_db, tmp_path, monkeypatch):
    ...
    def test_fetch_audiodb_for_artists_dedups_by_canonical_name(self, isolated_db, tmp_path, monkeypatch):
        monkeypatch.setattr("app.services.audiodb_service.ARTIST_COVERS_DIR", tmp_path)
    ...
    def test_fetch_audiodb_for_artists_calls_progress_callback(self, isolated_db, tmp_path, monkeypatch):
        monkeypatch.setattr("app.services.audiodb_service.ARTIST_COVERS_DIR", tmp_path)
...
class TestAudiodbSlug:
    ...
    def test_canonical_artist_name(self, input_name, expected):
    ...
    def test_audiodb_slug(self, canonical, expected_slug):
