...
def test_record_playback_event_writes_row():
    ...
    rows = list(MetadataDB._instance.execute(
...
def test_skipped_early_is_true_for_short_play():
    ...
    row = MetadataDB._instance.execute("SELECT skipped_early FROM playback_events").fetchone()
    assert row[0] == 1


def test_skipped_early_is_true_for_low_ratio():
    MetadataDB.record_playback_event(
        session_id="sess-1", collection_name="music", track_id="t1",
        played_sec=60.0, total_dur=240.0,   # 25% — below 30% threshold
    )
    row = MetadataDB._instance.execute("SELECT skipped_early FROM playback_events").fetchone()
    assert row[0] == 1
