...
class TestSearchServiceResolveModel:
    ...
    def test_collection_setting_wins_over_user_default(self, svc):
    ...
    def test_resolves_from_user_when_no_collection_setting(self, svc):
        _seed_user("acct-1", "Qwen/Qwen3-Embedding-0.6B")
        assert svc._resolve_model_name("acct-1", "no-such-col") == "Qwen/Qwen3-Embedding-0.6B"

    def test_resolves_from_collection_when_user_unset(self, svc):
    ...
    def test_falls_back_to_default_when_nothing_set(self, svc):
        assert svc._resolve_model_name("no-such-user", "no-such-col") == SearchService.DEFAULT_TEXT_MODEL

    def test_none_account_id_uses_collection_then_default(self, svc):
        MetadataDB.set_collection_text_model("col-X", "collection-pinned-model")
