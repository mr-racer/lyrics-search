...
class TestAuthGating:
    ...
    def test_gated_path_returns_401_without_token(self, gated_client, method, path):
        r = gated_client.request(method, path)
        assert r.status_code == 401, f"{method} {path} expected 401, got {r.status_code}"

    @pytest.mark.parametrize("method,path", GATED_PATHS)
    def test_gated_path_accepts_valid_token(self, gated_client, method, path):
    ...
    def test_instance_config_no_auth_required(self, gated_client):
        r = gated_client.get("/api/v1/instance/config")
        assert r.status_code == 200

    def test_login_no_auth_required(self, gated_client):
        r = gated_client.post(
            "/api/v1/auth/login",
            json={"email": "owner@example.com", "password": "ownerpass12345"},
        )
    ...
    def test_health_no_auth_required(self, gated_client):
        r = gated_client.get("/health")
        assert r.status_code == 200

    def test_root_no_auth_required(self, gated_client):
        r = gated_client.get("/")
        assert r.status_code == 200
