...
> фронт (стрим в плеере + ForYouHero + Recommend-таб) shipped 2026-06-11 (RS-2 в §9).**
...
- `GET /recommend/profile` — объяснимый long-term вкус: 6 осей (z+уровень), уверенность,
  острова (слитые якоря с member-обложками), кэшированный LLM-портрет/имена, liked_share.
- `POST /recommend/profile/ai-enrich` — портрет + имена островов одним LLM-вызовом
  (кэш `recsys_llm_texts`, само-инвалидация по хэшу состава островов).
- `POST /recommend/ai-playlist` — желание → план (clap_search/library_search/similar_tracks)
  → исполнение → курируемый отбор с reason-строками; деградация на каждом шаге.
- `POST /recommend/axis-playlist` — ранжирование коллекции под таргетные z ручек радара.
- `GET /recommend/similar?track_id=` — CLAP-соседи + ре-ранк по близости осей к сиду.
- `app/services/stream_service.py` — reward model / профили / пулы / скоринг / сборка;
  `app/services/recsys_ai_service.py` — LLM-слой.
...
> ### Status snapshot (updated 2026-06-12)
>
...
> | **MA-A** Multi-Account: Auth Foundation | ✅ **Shipped (2026-06-02)** — `users`/`invites`/`instance_config` SQLite tables + `AuthService` (argon2id + HS256 JWT, 30-day TTL, 7-day single-use invites) + `/auth/*` (login/register/me/logout/invites) + public `/instance/config` + FastAPI `get_current_user` gate on every `/api/v1/*` (except `/auth`, `/instance/config`, `/health`, `/`, `/covers`) + `scripts/create_owner` CLI (atomic owner + instance-mode lock) + frontend `LoginScreen`/`RegisterScreen` + global `apiFetch` Bearer/401-redirect + Settings Account/Invites panels. Instance mode `sharing`\|`server` locked at first start. **`collection_name` still client-supplied — Phase MA-D hides it.** | `docs/superpowers/plans/2026-06-02-phase-a-auth-foundation.md`, spec `docs/superpowers/specs/2026-06-02-multi-account-architecture-design.md` |
> | **MA-B** Multi-Account: singleton fixes | ✅ **Shipped (2026-06-06)** on `feat/phase-b-singleton-fixes` — stateless `LyricsSearchEngine.search(model_name=…)` (resolves via `ModelRegistry.get_text_model` per call under a per-model load lock; no more `self._model`/`_vector_name` mutation in the search path) + `SearchService._resolve_model_name` (collection_settings → user → default); per-account indexing queue `_active_jobs: dict[account_id, job_id]` + lock (replaces the single `_current_job_id`; same account → 409, distinct accounts run in parallel) gated by env-tunable global `MAX_PARALLEL_INDEXING_JOBS` semaphore (default 2); thread-safe per-collection classifier cache (`_classifier_cache_lock`, double-checked load); per-account transcoded cache (`cache/transcoded/<account_id>/<id>.flac`, keyed by collection_name today); server-side `users.text_model_name`/`clap_enabled` replacing `localStorage.text_model`. Also killed the last `db.lyrics_db._model` mutation (in `/chat`). **Task 12 (per-account `/stream` ownership guard, spec §6.5) deferred to MA-D** — its `acct_<id>` collection check only becomes meaningful once MA-D renames collections; enabling it today would 403 all streaming. | `docs/superpowers/plans/2026-06-02-phase-b-singleton-fixes.md` |
> | **MA-C** Multi-Account: indexing & storage (server mode) | 🛠 **Server-mode shipped (2026-06-06)** on `feat/phase-c-server-uploads` — `pending_uploads` table + content-addressed `media/<account_id>/audio/<sha>.<ext>` storage with `_quarantine/` staging; `uploads_service` (stream→quarantine→SHA-256→atomic promote, libmagic MIME gate, idempotent per (account, sha)); `IndexingService.index_uploads` → `acct_<id>` collection; routes `POST /library/upload`, `GET /library/upload/{id}`, `POST /library/upload/batch-commit`, `DELETE /library/tracks/{id}` (all `require_mode("server")`); `POST /library/index` gated to sharing mode (**revised 2026-06-13: allowed in server mode for the OWNER, 403 for members — see FIX row below**); hourly quarantine+pending_uploads cleanup loop; owner CLI `scripts/hardlink_owner_library.py`; mode-aware `OnboardingScreen` + `ServerOnboardingScreen` upload wizard. `MetadataDB` got `PRAGMA busy_timeout` for concurrent-write safety. **Deferred to MA-D/E (per the Phase-B Task-12 decision): the `/stream` switch to `acct_<id>` derivation + 410-Gone-on-broken-link, and the live-CLAP e2e round-trip tests.** Sharing-mode legacy reads stay on the query-param collection. | `docs/superpowers/plans/2026-06-02-phase-c-indexing-storage.md` |
> | **MA-D** Multi-Account: hide collection_name | ✅ **Shipped (2026-06-10)** on `feat/phase-d-hide-collection-name` — every public endpoint now derives the Qdrant collection strictly from the JWT user as `acct_<user.id>` via the single source of truth `app/api/helpers.derive_collection_for_user`; clients never supply it. **D-soft** relaxed each `collection_name`/`collection` param to optional-and-ignored (logged via `deprecated_collection_warning`) across `/search`, `/library`, `/metadata`, `/playback`, `/playlists`, `/recommend`, `/chat`, `/library/ai-index`, `/artists`; renamed `/library/collections/{name}/settings\|ai-enabled` → `/library/settings\|ai-enabled` (old paths 410); self-serve `DELETE /library/collection/{name}` → 410, replaced by owner-only `POST /admin/accounts/{user_id}/wipe` (gated by `get_owner`, mirrors the old delete's cache-purge parity). **Frontend** stopped sending collection in every fetch URL/body, swapped the COLLECTIONS picker for a read-only `Library: <email>` row, removed the `activeCollection` state entirely, repointed `useChatHistory`/`useRecentSearches`/`useTrackChat` to `user.id`, and runs a one-time idempotent localStorage migration (`runCollectionLocalStorageMigration_d`, marker `musix_localstorage_migration_d`) re-suffixing `acct_<id>` keys → `user.id`. **D-hard** then dropped `collection_name`/`collection` from every request Pydantic model — graceful: no `extra='forbid'`, so a stale client's field is ignored (200), not 422 — and removed the deprecated query params + warning calls; response models that echo `collection_name` are kept. Per the rollout protocol D-hard is the final, independently-revertible commit (ship it a cycle after D-soft+frontend soak). Adds E2E cross-account isolation test + a node localStorage-migration unit test. | `docs/superpowers/plans/2026-06-02-phase-d-hide-collection-name.md` |
> | **RS-2** Stream RecSys frontend + Recommend tab | ✅ **Shipped (2026-06-11)** on `feat/stream-recsys` — **Стрим в плеере**: режим потока в App (чанки `GET /stream/next` n=3, префетч 1–2 трека как единственный лаг адаптации, `exclude_ids` покрывает выданный-несыгранный хвост, сильный сигнал лайк/дизлайк/скип обрезает префетч и рефетчит уже с пост-сигнальным профилем, ручной запуск любого трека выходит из режима); флаг `interacted` (реакция/скип/пауза/перемотка/прыжок по очереди помечают текущее прослушивание через `el.dataset`, уходит с каждым playback-событием → активирует idle rule); `setSrc` идемпотентен для того же трека (попутно починен сброс аккумулятора при ремаунте секции). **ForYouHero** на `/recommend/profile`: ведущий якорь + лента обложек островов + слайдер любимые/новое (debounced `PUT /stream/settings`, сервер читает на каждом чанке) + честный cold-start «Начнём с разведки». **Recommend-таб с нуля**: зона A — «одно желание → плейлист» (`POST /recommend/ai-playlist`: план → исполнение → отбор; шаги анимируются, у треков reason-строки; при AI-off — quick-mix пресеты); зона B — центр вкуса: управляемый 6-осевой радар (полигон профиля + пунктир ручек, `POST /axis-playlist`, сброс к профилю), LLM-портрет слушателя (↺ `POST /profile/ai-enrich`, кэш `recsys_llm_texts` по хэшу островов), карточки островов вкуса → радио от якоря; зона C — похожие через `GET /recommend/similar` (CLAP top-K + ре-ранк по осям 0.7/0.3) вместо старого текстового хака. Новые эндпоинты: `/recommend/{profile, profile/ai-enrich, ai-playlist, axis-playlist, similar}`. | spec `docs/2026-06-09-stream-recsys-design.md` |
> | **FRW** First-Run Setup Wizard | ✅ **Shipped (2026-06-11)** on `feat/first-run-setup-wizard` — web bootstrap `POST /instance/setup` (public while uninitialized, first-come-first-served, 409 forever after; built on new atomic `AuthService.bootstrap_instance` now shared with `scripts/create_owner` CLI). Full-screen staged `SetupWizard` replaces the bare 404 boot error: welcome → owner account → mode choice (lock warning) → music (folder / drag-drop by mode) + **3-tier embedder slider** (jina-small en · **new** `intfloat/multilingual-e5-base` 768d Balanced · Qwen3-0.6B) → optional AI (live `/system/llm-status` probe, skippable) → SSE stage progress → app handoff without reload. RU/EN + theme toggles on every step (`musix_lang`/`musix_theme` carry into the app). Password policy `MIN_PASSWORD_LENGTH` 10→6 (global). `text_model` now honored by server-mode `POST /library/upload/batch-commit` (threaded into the upload job + pinned in `collection_settings`, mirroring the folder flow). Gates: `scripts/check_jsx.py` (Babel parse), `scripts/smoke_setup_wizard.py` (parse + ident grep + live 404→200→409 round-trip on temp DB). | spec `docs/superpowers/specs/2026-06-10-first-run-setup-wizard-design.md`, plan `docs/superpowers/plans/2026-06-11-first-run-setup-wizard.md` |
> | **FIX** Auth for headerless browser APIs | ✅ **Shipped (2026-06-12)** — post-FRW hotfix wave: (1) **stream token** — `<audio>` can't send `Authorization`, so playback 401'd everywhere since MA-A; new scope-limited 1h JWT (`AuthService.issue_stream_token`/`verify_stream_token`, mutual scope rejection with login tokens), `POST /auth/stream-token`, `stream_router` carved out of the blanket gate with its own `get_user_for_stream` (Bearer OR `?st=`), frontend `buildStreamUrl` + module cache refreshed on App mount/20-min interval, dropped on logout; (2) **playback events** — `navigator.sendBeacon` can't send headers either → every listen event 401'd and the Поток reward signal was silently empty; replaced with `fetch(keepalive:true)` + Bearer; (3) **wizard LLM probe** — raw `fetch` on gated `/system/llm-status` showed the 401 body as the probe result; now `apiFetch`; (4) **401-kick loop guard** — `_onAuthFailure`'s `#login`-hash check left the app mounted token-less after a re-login (hash never cleared) → silent logout cascade; guard moved to a 5s `sessionStorage` timestamp, reset by `setStoredAuth`. Tests: `tests/unit/test_stream_token.py` (13) + `tests/integration/test_api_stream_auth.py` (5). | inline (`app/services/auth_service.py`, `app/api/dependencies.py`, `frontend/index.html`) |
> | **FIX** Cover CORS cache + stream slider integrity | ✅ **Shipped (2026-06-12)** — (1) **cover color / equalizer purple fallback**: cover responses are cached immutable 1y; a copy first fetched by a plain `<img>` (no `Origin` → CORSMiddleware skips) had no ACAO and poisoned the cache for the later `crossOrigin='anonymous'` canvas read in `useCoverColor` → CORS error → purple. `serve_cover` now sends `Access-Control-Allow-Origin: *` unconditionally; `useCoverColor` samples remote covers via a separate `?cors=1` cache key (heals already-poisoned caches). (2) **liked/new slider strict zero**: `assemble_chunk` topped up from the liked pool beyond quota when the main pool ran dry — in small libraries (liked + session-played excluded from anchor/explore) this made the slider's «новое» extreme a no-op; at `liked_share == 0` the topup is now skipped (short chunk честнее). (3) **silent collection default removed**: `LibraryService.index_folder` required a name only optionally (`= "music_explorer"`); a pre-Phase-D wizard indexing run hit that default and produced a library invisible to every `acct_*`-derived route (the 2026-06-12 «статистика пропала / ползунок мёртв» incident) — the param is now required. Tests: `test_covers_endpoint.py` (ACAO with/without Origin), 2 strict-zero cases in `test_stream_pools_scoring.py`. | inline (`app/api/main.py`, `app/services/stream_service.py`, `app/services/library_service.py`, `frontend/index.html`) |
> | **FIX** Server-mode wizard bug wave (SQLite threads · upload progress/covers · wizard folder) | ✅ **Shipped (2026-06-13)** — (1) **auth 401/500 loop**: `MetadataDB` held ONE `sqlite3.Connection` (`check_same_thread=False`) shared by every FastAPI threadpool worker; concurrent `get_current_user` lookups raced inside CPython's sqlite3 (`InterfaceError: bad parameter or other API misuse` → 500) or returned None rows → «unknown user id» → 401 → frontend logout loop after every post-index request burst (covers/facts/sonic-vibe per track), включая stream-токены. Fixed with per-thread connections (`_connections: {thread_id: (db_path, conn)}`, re-opened on DB_PATH change; `close()` closes all + legacy `_instance`); WAL+busy_timeout уже были. Repro test: `tests/unit/test_metadata_db_thread_safety.py` (8 потоков × 400 итераций — до фикса ловит обе моды). (2) **upload-indexing silent until embeddings + no covers**: `IndexingService.index_uploads`' per-file loop (tag read + online lyrics fetch — самая долгая часть) не звал `progress_callback` и не извлекал обложки; теперь шлёт `("scan", i, total, …)` (новый маппинг scan→LYRICS в `_on_index_progress`, PENDING→RUNNING promotion на первом событии) + tqdm + `save_cover_art` per file (паритет с folder-флоу `_metadata_to_tracks`). (3) **wizard folder in server mode**: `/library/index` больше не sharing-only — в server mode owner-only 403-гейт для членов (members продолжают грузить через upload); шаг «Музыка» в визарде показывает строку папки в обоих режимах + зону загрузки в server (picked files приоритетнее). Mode-gate тесты обновлены (owner 200/503, member 403). | inline (`app/resources/metadata_db.py`, `app/services/indexing_service.py`, `app/services/library_service.py`, `app/api/routes/library.py`, `frontend/index.html`) |
> | **FIX** Atlas hero photo treatment + artist-image route | ✅ **Shipped (2026-06-13)** — (1) **/covers/artists/ never resolved**: artist images live one segment deeper than album covers, а `serve_cover`'s `{cover_file}` не матчит «/», так что `${API}/covers/artists/<hash>.png` проваливался в SPA catch-all и возвращался как index.html (вечно битый `<img>` в герое Атласа); новый роут `serve_artist_cover` с тем же контрактом cache/ACAO + filename-only guard (отказ от `%5C`-траверсала на Windows). (2) **не-cutout фото в герое**: AudioDB иногда кладёт прямоугольный JPEG в слот cutout — жёсткие края, верх фото под хлебными крошками. Новый хук `useImageHasAlpha` (канвас-проба 24×24 на прозрачность; .jpg → сразу «фото»; ошибка → «фото») делит рендер на три ветки: настоящий cutout — как раньше; прямоугольное фото — «растворение маской» (горизонтальная маска на обёртке × вертикальная на img — все четыре края уходят в blur-фон из той же картинки = ambient-заполнение краевыми цветами; верхний fade освобождает строку крошек; без drop-shadow); нет картинок — монограмма. Foreground теперь падает обратно на thumb_path (Deezer-фото), а не сразу на монограмму. Tests: 2 новых в `test_covers_endpoint.py`. | inline (`app/api/main.py`, `frontend/index.html`) |
> | **MA-E** Multi-Account: cleanup & migration | ⏳ Not started — plan drafted | `docs/superpowers/plans/2026-06-02-phase-e-cleanup-migration.md` |
> | **UXR** Post-design-review UX + recsys refinement | 🛠 **Code-complete (2026-06-20), pending manual E2E + mirror to `container-version`** on `refine/post-design-review-2026-06-20` — four-part polish after a design review. **(1) Jargon removal** (`frontend/index.html`, RU+EN): «индексация»→«Добавить музыку»/«Заменить музыку» (first-run vs re-run), «коллекция»→«библиотека», «Qdrant/векторная база»→«Поиск временно недоступен», rail header «CLAP+ОСИ»→«ПО ЗВУЧАНИЮ», hid BM25, «LLM»→«ИИ-ассистент», «AI-индексация артефактов»→«Дополнительные ИИ-функции», dropped «эмбеддинги», phases→«Теги и обложки/Тексты песен/Звучание/Похожие треки»; display strings only (code identifiers untouched). **(2) Off-player auto-advance**: the only `'ended'` auto-advance listener was gated to the player screen (`audio` prop player-only), so a track ending on Home/«For You» advanced nothing; added an App-level listener active when `section!=='player'` (mutually exclusive with PlayerSection's) + made `ensureAutoplayQueue` return its appended hits so `advancePlaybackOffPlayer` rolls into a non-stream refill. **(3) Sub-30s + same-album filters**: new `stream_service._duration_ok` (drop iff `0<dur<30`, keep unknown) wired into `next_chunk` (all 4 pools), `similar_tracks`, `autoplay_service`; player «ПОХОЖИЕ» rail now drops same-album-as-seed neighbours (similar only; contrast keeps) and `<30s` via `build_track_pairs` (filter-before-cap, backfill to 3), with `analyze_collection` excluding `<30s` tracks at build + storing top-12. **(4) Queue + influence**: rail «queue next» now inserts into App's `playerPlaylist` single source of truth (was clobbered by PlayerSection's prop re-sync → skipped) with metadata enrichment; new `playback_events.influence` column (default 1) carried frontend→backend via `el.dataset.playNoInfluence`, so a hand-queued track is excluded from the «For You» taste profile (`next_chunk` AND `long_term_profile`) while still counting for anti-repeat. Gates: `check_jsx.py`, `pyflakes`/`py_compile`, targeted `pytest` (new: `test_stream_duration_filter`, `test_playback_influence`, +3 `build_track_pairs` cases), `smoke_phase_a.py`. **Follow-up (2026-06-20)**: «ПОХОЖИЕ» всё равно проседало до 0–2, потому что same-album drop жил ТОЛЬКО на чтении и грыз уже обрезанный до 12 кеш (одноальбомные треки — ближайшие по CLAP → забивали верх; 13-й+ сосед в кеш не попадал, добрать неоткуда), а проседание «КОНТРАСТ» — признак устаревшего `top_k=5` кеша до отсева коротких на записи. Fix: same-album exclusion перенесён в `get_top_pairs` на стадию ЗАПИСИ (`drop_same_album_similar` kw; scan-deeper backfill до 12 different-album similars; `analyze_collection(...,drop_same_album_similar=True)`), read-time фильтр оставлен safety-net; home-витрина не затронута (`/library/top-pairs?sample=24` читает лишь top-level `dissimilar`). Действует после пересборки кеша — новый `scripts/recompute_similarity.py --email <…>` гоняет `analyze_collection` для одного аккаунта БЕЗ переэнкодинга (читает готовые CLAP-векторы). Tests: +4 build-time album-filter cases (23/23 в `test_similarity_math.py`, RED→GREEN). | spec `docs/superpowers/specs/2026-06-20-ux-backend-refinement-design.md`, plan `docs/superpowers/plans/2026-06-20-ux-backend-refinement.md` |
> | **PSR** Player similarity rail (похожие/контраст под очередью) | ✅ **Shipped (2026-06-20)** on `feat/player-similarity-rail` — под блоком «Очередь» на Player две колонки: топ-3 **похожих** (emerald) + топ-3 **контраст** (amber) по CLAP-аудио из предрассчитанного top-pairs кеша. Новый лёгкий `GET /library/top-pairs/{track_id}` (мемоизированный по mtime `track_id→соседи` индекс в `similarity_service.load_top_pairs_index`; обогащение ≤6 соседей одним `qdrant.retrieve` → чистые title/artist/genre/cover + score via `build_track_pairs`) отдаёт лишь текущий трек — КБ, не вся 10MB-матрица. Карточка: мини-обложка + title/artist (ellipsis, `minWidth:0`) + тонкий % сходства; клик → **вставка следующим** (`currentIndex+1`, без `_autoplay` — не конфликтует с автоподбором, дописывающим в конец при исчерпании). Очередь получила `minHeight≈3 трека` (адаптив на низких экранах); блок скрыт при `available:false` и при открытом AI-чате. Гейты: `build_track_pairs`/`load_top_pairs_index` unit/integration (red→green в изоляции — локальный pytest падает на torch-импорте `app/__init__`), `check_jsx.py` green, smoke-чек роута. | spec `docs/superpowers/specs/2026-06-20-player-similarity-rail-design.md`, plan `docs/superpowers/plans/2026-06-20-player-similarity-rail.md` |
>
...
>
> **Out-of-plan addition (2026-06-12)**: Deezer image fallback inside the same FACTS-stage enrichment (`audiodb_service._fetch_deezer_picture`). When AudioDB yields no downloadable artist image (artist unknown, or both cutout/thumb downloads failed), `api.deezer.com/search/artist?q=<lowercase+plus slug>` is queried; the first result is accepted only on a case-insensitive exact match of `name` against the pre-slugify canonical artist name, then square `picture_xl` is downloaded into the shared `/covers/artists/` content-addressed cache and persisted as `thumb_path`. Total AudioDB network failure still writes no row (preserves next-run retry), so Deezer is not consulted in that case.
...
>
> **Atlas v2 redesign (2026-06-12, out-of-plan polish round)** — ✅ full `ArtistAtlasSection` rewrite: tabs replaced by a **single-scroll page with anchor navigation**. Immersive hero (AudioDB `thumb_path` blurred backdrop + `cutout_path` artist photo, fallback = blurred first-album cover + serif monogram; first frontend consumer of the 2026-05-20 AudioDB fields), **liquid-glass dock** (anchor pills БИО·ФАКТЫ·АЛЬБОМЫ + Spin CTA, condenses into a sticky glass strip past the hero), bio with clamp/«читать дальше», **facts shelf** (horizontal snap-scroll card-index cards with numbered counters, arrows + progress dots), **albums timeline** (covers on a year-proportional axis with ticks, fallback to airy row when <3 dated albums or zero year spread; vinyl-disc slides out of the sleeve on hover, reusing `.vinyl-disc` from the Home deck). Tracks no longer spill onto the page — covers open the shared `AlbumModal` gatefold with FLIP fly-out. **Gold star ★** on albums with 3+ liked tracks, fed by new `ArtistAlbum.liked_track_count` (single `get_reactions_for_tracks` join in `build_artist_aggregate`; `/library/featured-artist` gets it for free). Spec: `docs/superpowers/specs/2026-06-12-artist-page-redesign-design.md`. New frontend pieces: `Atlas*` component family + `.atlas-*` CSS block. Empty-data degradations: missing bio/facts/albums sections (and their anchor pills) simply don't render.
