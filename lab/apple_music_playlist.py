"""
Парсер страниц плейлистов Apple Music (music.apple.com/*/playlist/*).

На странице ТРИ независимых источника данных, и ни один не самодостаточен:

  1. JSON-LD  <script id="schema:music-playlist" type="application/ld+json">
     Есть всегда, в т.ч. в сыром ответе requests.
     Даёт: name, author, url, numTracks, datePublished + по треку name/url/duration.
     НЕ даёт: исполнителя, альбом, настоящее описание (там SEO-заглушка).

  2. serialized-server-data  <script id="serialized-server-data" type="application/json">
     Payload SvelteKit'а. Есть в сыром ответе requests — главный источник,
     когда страница не отрендерена браузером.
     Даёт: title, artistName/subtitleLinks, tertiaryLinks (альбом),
     duration в мс, composer, showExplicitBadge, обложку, adam-id.

  3. DOM (разметка songs-list с data-testid)
     Есть только после гидратации (сохранённая из браузера страница / headless).
     Даёт то же, что и (2).

Приоритет при склейке: DOM > serialized > JSON-LD. Ключ склейки — числовой
adam-id трека, поэтому порядок и рассинхрон между источниками не важны.
...
class Track:
    ...
    def artist(self) -> str | None:
        """Исполнители одной строкой, как их показывает Apple."""
        return " & ".join(self.artists) or None

    def fill_from(self, other: "Track") -> None:
        """Дозаполнить пустые поля из другого источника (self приоритетнее)."""
...
class Playlist:
    ...
    title: str | None = None
    author: str | None = None           # куратор: «Apple Music Alternative»
    author_url: str | None = None
    description: str | None = None      # настоящее редакторское описание
    seo_description: str | None = None  # то, что лежит в JSON-LD/og:description
    url: str | None = None
    artwork: str | None = None
    date_published: str | None = None
    num_tracks: int | None = None
    sources: list[str] = field(default_factory=list)  # какие источники сработали
...
def _song_id_from_url(url: str | None) -> str | None:
    """Apple использует два формата: /song/<slug>/<id> и /album/<slug>/<aid>?i=<id>."""
...
# --------------------------------------------------------------------------- #
#  Источник 2: serialized-server-data
# --------------------------------------------------------------------------- #

def _extract_serialized(doc) -> list[Any]:
    """Все JSON-payload'ы SvelteKit со страницы."""
    payloads = []
...
def _links(obj: dict, key: str, kind: str) -> tuple[list[str], list[str]]:
    """Из subtitleLinks / tertiaryLinks достаёт (названия, adam-id)."""
    names, ids = [], []
...
def _serialized_header(payloads: list[Any]) -> dict[str, Any]:
    """
    Шапка плейлиста в payload'е. Форма у Apple плавает между релизами,
    поэтому ищем эвристически: самое длинное осмысленное описание,
    не являющееся SEO-заглушкой.
    """
    out: dict[str, Any] = {}
    best_desc = None
    for payload in payloads:
...
def _dom_header(doc) -> dict[str, Any]:
    ...
    # Обрезка описания там чисто CSS-ная — в DOM лежит полный текст
...
def _merge(*sources: list[Track]) -> list[Track]:
    """
    sources — по убыванию приоритета. Порядок берётся из первого непустого
    источника, недостающие поля дозаполняются из остальных по song_id.
    """
    ...
    # треки, которых нет в базовом источнике (например, догруженные скроллом)
...
def parse_apple_playlist(html: str | bytes, url: str | None = None) -> Playlist:
    """
    Разбирает HTML страницы плейлиста Apple Music.

    :param html: сырой HTML (str или bytes) — из requests или сохранённый из браузера
    :param url:  URL страницы, запасной источник id и ссылки
    """
    ...
    # JSON-LD задаёт канонический порядок треков, поэтому если он есть —
    # он становится базой, а DOM/serialized дозаполняют поля.
