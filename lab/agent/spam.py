...
A broken engine does not fail cleanly. It returns a page of adult-cam or
casino affiliate spam, SearXNG fuses that with the good engines' results by
rank, and the two interleave — one real link, one brothel, one real link. From
inside the pipeline the spam looks exactly like a search result: it has a
title, a snippet and a rank, so it survives to the cross-encoder, sometimes
past it, and occasionally into an answer.
...
spam counter in ``sources`` is what actually identifies the engine to remove.
"""
...
# bare substring also sits inside "scams", which a real site can be about.
# One lookbehind is cheaper and clearer than an exception list.
...
def is_spam_host(url: str) -> bool:
    ...
    # hyphen still reads as one word: "strip.chat" and "chatur-bate.com" are
    # the same trick as "stripchat.com".
...
def spam_report(urls) -> dict[str, int]:
