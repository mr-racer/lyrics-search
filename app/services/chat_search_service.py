"""Agentic LLM-driven music search — the engine behind POST /chat and the
assistant's ``search`` intent.

Moved verbatim out of ``app/api/routes/chat.py`` so it can be called without a
FastAPI ``Request``: the route only ever passed ``request.app.state.search_service``
through, and the unified assistant needs the same core. ``routes/chat.py`` is now
a thin wrapper over :func:`run_chat_search` and its public contract is unchanged.

Flow per user message
─────────────────────
Call 1  (classification, or the PydanticAI Planner when ``planner_enabled``)
    → query type (text / audio / hybrid) + filters + initial queries

Call 2…N  (agentic search loop, up to NUM_ATTEMPTS)
    • "search" → run the LLM's queries against the library, accumulate
                 context, repeat.
    • "answer" → return the LLM's message + all retrieved hits.

Audio queries take a fast path instead: CLAP rephrase ×3 → audio search → RRF
merge → one conversational answer.
"""
...
from app.domain.models import SearchFilters, TrackHit, User
    ...
    for q in llm_queries:
        ...
        if not skip_rephrase and mode in ("audio", "hybrid") and _CLAP_REPHRASE_SYSTEM_PROMPT.strip():
            try:
                ...
                logger.warning("[chat] CLAP rephrasing in agentic loop (non-fatal): %s", exc)
...
# ─── Agent step events ────────────────────────────────────────────────────────
# The streaming callers drain these as the agentic loop progresses. Each event
...
# non-streaming path, which makes every _emit() call a cheap no-op.
...
async def _emit(emit: EmitFn | None, event_type: str, lang: str | None, **fields) -> None:
    if emit is None:
        return
    await emit({"type": event_type, "human": human(event_type, lang, **fields), **fields})
...
async def run_chat_search(
    ...
    if needs_old_classifier:
        try:
            ...
            logger.error("[chat] classification error (non-fatal): %s", exc, exc_info=True)
    ...
    for attempt in range(1, NUM_ATTEMPTS + 1):
        ...
        # ScorerAgent evaluates accumulated context
            ...
            try:
                ...
                logger.error("[chat] ScorerAgent error on attempt %d: %s", attempt, exc,
                             exc_info=True)
        ...
        if action is None:
            ...
            try:
                ...
                logger.error("[chat] LLM error on attempt %d: %s", attempt, exc, exc_info=True)
                final_result = {
                    "action":     "answer",
                    "confidence": "low",
                    "song":       None,
