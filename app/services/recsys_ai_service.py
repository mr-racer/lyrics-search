...
def build_playlist_response(result: dict):
    """Shape the :func:`ai_playlist` dict into the ``AIPlaylistResponse`` contract.

    Shared by the plain route, the streaming route and the unified assistant —
    it lives here rather than in ``routes/recommend.py`` so a service never has
    to import a route module to reuse it.

    The candidate dicts carry Qdrant-payload fields only, and the credits live
    in SQLite — hence the same ``apply_song_relations`` overlay the wave and
    /metadata/tracks do. Without it the player showed no producers chevron and
    no samples pill for AI playlists: these tracks come with a real
    ``file_path``, so the client's slim-shape enrichment (triggered by a FALSY
    file_path) never rescued them either.
    """
    from app.domain.models import AIPlaylistResponse, AIPlaylistStep, AIPlaylistTrack
