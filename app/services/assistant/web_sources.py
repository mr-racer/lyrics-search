...
def engine_host_spread(results: list) -> dict:
    ...
    return {name: {"results": e["results"], "hosts": len(e["hosts"]),
                   "top_host": (max(e["hosts"], key=len, default="")
                                if len(e["hosts"]) == 1 else "")}
            for name, e in sorted(out.items(), key=lambda kv: -kv[1]["results"])}


def dominating_host(results: list, *, min_share: float,
...
class SearchSources:
    ...
    def _searx(self, query: str, *, engines: Optional[str], limit: int,
               force: bool = False, host_pinned: bool = False) -> list:
    ...
    def _clean(self, query: str, data: dict, *, engines: Optional[str],
               limit: int, host_pinned: bool, asked) -> list:
        ...
        if dominant:
            ...
            logger.warning(
                "[sources] %r took over the results for %r — dropped. Engine(s) "
                "dumping it: %s", dominant, query,
                culprits or "(not reported; see engine_spread)")
            self._emit("host_takeover", query=query, host=dominant,
        ...
        if spam_rows:
            ...
            self._emit("spam_dropped", query=query, count=len(spam_rows),
                       engines=entry["spam_by_engine"])
    ...
    @staticmethod
    def _to_hits(rows: list, source: str,
                 *, host_filter: Optional[tuple] = None) -> list:
        out: list = []
    ...
    def _pinned_to(self, source: str, domain: str, query: str, *, limit: int,
                   host_filter: tuple, content_path: tuple = ()) -> list:
        ...
        """
        rows = self._searx(f"site:{domain} {query}", engines=None,
                           limit=self.cfg.searx_pool, host_pinned=True)
        ...
        for hit in self._to_hits(rows, source, host_filter=host_filter):
            ...
            if content_path and not any(m in path for m in content_path):
                logger.debug("[sources] %s: %s is not a content page", source,
                             hit.url)
                continue
            hits.append(hit)
        hits = hits[:limit]
    ...
    def apple_music(self, query: str, limit: int = 3) -> list:
        return self._pinned_to("apple", "music.apple.com", query, limit=limit,
                               host_filter=("music.apple.com",),
                               content_path=("/album/", "/playlist/",
                                             "/artist/", "/song/"))

    def fandom(self, query: str, limit: int = 2) -> list:
        return self._pinned_to("fandom", "fandom.com", query, limit=limit,
    ...
    def reddit(self, query: str, limit: int = 3) -> list:
        ...
        """
        return self._pinned_to("reddit", "reddit.com", query, limit=limit,
...
def rerank_hits(hits: list, ce_query: str, *, hub, threshold: float) -> list:
    ...
    logger.info("[sources] cross-encoder kept %d/%d at p>=%.2f", len(kept),
                len(hits), threshold)
    return kept
