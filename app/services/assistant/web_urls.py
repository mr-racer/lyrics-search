...
Two search streams routinely return the same article, and SearXNG hands the same
Wikipedia page back in several spellings: percent-encoded and not
(``Taylor_Swift%E2%80%93Kanye_West_feud`` vs ``Taylor_Swift–Kanye_West_feud``),
with and without ``www.``, sometimes through the ``m.`` mobile mirror, often with
tracking parameters glued on.
...
lowercases the host, which is right for comparison and wrong for an HTTP request;
the original URL is always what goes over the wire.
"""
...
def source_for_url(url: str, fallback: str = "web") -> str:
    ...
    Not from the search stream that found it. A Wikipedia article is a Wikipedia
    article whether the ``wikipedia`` engine returned it or Google did — and the
    difference matters twice over: structured extraction only runs for the kinds
    that have parseable structure, and the source weight only doubles for the
    kinds worth double.

    Labelling by stream is what made a 900-row discography table yield nothing:
    the page came back from the open-web search, was labelled "web", and the table
...
# ordering differ per country, and search hands back whichever one it happened to
# index — /vc/ (Vatican City) as readily as /us/. Pinning it makes runs
...
def normalise_apple_url(url: str) -> str:
    ...
    * it mixes in "Appears On" — "Run This Town" is a JAY-Z song and "Knock You
      Down" is Keri Hilson's, and in the serialized payload they sit next to the
      artist's own with nothing to tell them apart;
...
def dedupe_by_url(items, *, key=lambda item: item.url):
    ...
    Order is preserved because the caller ranked these, and the first spelling of
    a URL to arrive came from the better-ranked result.
    """
    seen: set = set()
    out = []
    for item in items:
